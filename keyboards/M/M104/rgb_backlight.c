...
#if defined (RGB_BACKLIGHT_ZEAL60) || defined (RGB_BACKLIGHT_ZEAL65) || defined (RGB_BACKLIGHT_M60_A) || defined(RGB_BACKLIGHT_M6_B) || defined(RGB_BACKLIGHT_KOYU) || defined(RGB_BACKLIGHT_HS60) || defined(RGB_BACKLIGHT_WS2812)
#else
#error None of the following was defined: RGB_BACKLIGHT_ZEAL60, RGB_BACKLIGHT_ZEAL65, RGB_BACKLIGHT_M60_A, RGB_BACKLIGHT_M6_B, RGB_BACKLIGHT_KOYU, RGB_BACKLIGHT_WS2812
#endif
...
#if defined(RGB_BACKLIGHT_WS2812)
...
#elif defined(RGB_BACKLIGHT_WS2812)
#include "rgblight.h"
#define BACKLIGHT_LED_COUNT RGB_BACKLIGHT_LED_NUM
#elif defined (RGB_BACKLIGHT_HS60)
...
backlight_config g_config = {
	...
	#if defined(RGB_BACKLIGHT_WS2812)
	.num_lock_indicator = RGB_BACKLIGHT_NUM_LOCK_INDICATOR,
	.scroll_lock_indicator = RGB_BACKLIGHT_SCROLL_LOCK_INDICATOR,
	#endif
...
#if defined(RGB_BACKLIGHT_M6_B) || defined(RGB_BACKLIGHT_WS2812)
	return;
#endif
...
void map_row_column_to_led( uint8_t row, uint8_t column, uint8_t *led )
{
	*led = 255;
#if defined (RGB_BACKLIGHT_WS2812)
  // this is fake and will not be used except for LED testing
  *led = (row * column) % BACKLIGHT_LED_COUNT;
#else
...
void backlight_effect_indicators(void)
{
	...
	#if defined(RGB_BACKLIGHT_WS2812)
	if ( g_config.num_lock_indicator.index != 255 &&
			( g_indicator_state & (1<<USB_LED_NUM_LOCK) ) )
	{
		backlight_effect_indicators_set_colors( g_config.num_lock_indicator.index, g_config.num_lock_indicator.color );
	}
	if ( g_config.scroll_lock_indicator.index != 255 &&
			( g_indicator_state & (1<<USB_LED_SCROLL_LOCK) ) )
	{
		backlight_effect_indicators_set_colors( g_config.scroll_lock_indicator.index, g_config.scroll_lock_indicator.color );
	}
#endif
...
#endif
{
	...
	//rgblight_sethsv_at( 64, 255, 10,0);
}
...
void backlight_config_set_value( uint8_t *data )
{
	...
	switch ( *value_id )
	{
		...
		case id_num_lock_indicator_color:
		{
			_set_color( &(g_config.num_lock_indicator.color), value_data );
			break;
		}
		case id_num_lock_indicator_row_col:
		{
			backlight_set_indicator_index( &(g_config.num_lock_indicator.index), value_data[0], value_data[1] );
			break;
		}
		case id_scroll_lock_indicator_color:
		{
			_set_color( &(g_config.scroll_lock_indicator.color), value_data );
			break;
		}
		case id_scroll_lock_indicator_row_col:
		{
			backlight_set_indicator_index( &(g_config.scroll_lock_indicator.index), value_data[0], value_data[1] );
			break;
		}
#endif
...
void backlight_config_get_value( uint8_t *data )
{
	...
	switch ( *value_id )
	{
		...
		case id_num_lock_indicator_color:
		{
			_get_color( &(g_config.num_lock_indicator.color), value_data );
			break;
		}
		case id_num_lock_indicator_row_col:
		{
			backlight_get_indicator_row_col( g_config.num_lock_indicator.index, &(value_data[0]), &(value_data[1]) );
			break;
		}
		case id_scroll_lock_indicator_color:
		{
		_get_color( &(g_config.scroll_lock_indicator.color), value_data );
			break;
		}
		case id_scroll_lock_indicator_row_col:
		{
			backlight_get_indicator_row_col( g_config.scroll_lock_indicator.index, &(value_data[0]), &(value_data[1]) );
			break;
		}
#endif		
		case id_layer_1_indicator_color:
...
#if defined(RGB_BACKLIGHT_WS2812)
//#TODO init RBG 
	// // Set RGB to static light
	rgblight_enable_noeeprom();
	rgblight_mode(1);
	rgblight_setrgb(0,0,0);
	// TODO: fix eeprom settings for indicators
	g_config.caps_lock_indicator.index = 1;
	g_config.num_lock_indicator.index = 2;
	g_config.scroll_lock_indicator.index = 0;
	g_config.layer_1_indicator.index = g_config.scroll_lock_indicator.index;
	g_config.layer_2_indicator.index = g_config.scroll_lock_indicator.index;
	g_config.layer_3_indicator.index = g_config.scroll_lock_indicator.index;
